...
import { Settings, Sparkles, ChevronsUpDown, ChevronRight, User, PenTool, Palette, Minus, Plus } from 'lucide-react';
...
export default function StepReview({
    ...
    handleOpenSearch, onApplyComposition
}: StepReviewProps) {
        ...
        <div className="max-w-[1600px] mx-auto flex gap-8 items-start animate-in fade-in">
            <div className="w-[340px] shrink-0 space-y-6 h-fit sticky top-24">
                <div className={`${containerBg} p-5 rounded-3xl space-y-6`}>
                    ...
                    {/* 模式切换 */}
                    <div className={`flex ${isDark ? 'bg-black' : 'bg-gray-100'} p-1 rounded-xl`}>
                    ...
                    {/* 🟢 [核心修改] 移除了场景一致性控制台 (Scene Consistency) 区域 */}

                    {/* 场景描述输入框 (纯文本，仅作 Prompt 补充，不带上传) */}
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-zinc-500 uppercase">
                            {t.scene || "GLOBAL CONTEXT"}
                        </label>
                        <div className={`flex items-center gap-2 ${inputBg} border ${isDark ? 'border-zinc-700' : 'border-gray-200'} px-3 py-2.5 rounded-xl focus-within:border-blue-500 transition-colors`}>
                            <input
                                value={sceneDescription}
                                onChange={(e) => setSceneDescription(e.target.value)}
                                placeholder={t.scenePlaceholder || "Common location (e.g. Cyberpunk City)..."}
                                className={`bg-transparent text-xs ${isDark ? 'text-white' : 'text-gray-900'} placeholder-zinc-500 outline-none w-full font-bold truncate`}
                            />
                        </div>
                    </div>

                    {mode === 'render' && (
                        <div className="space-y-4 pt-2 border-t border-dashed border-zinc-700/50">
                            {/* 氛围控制 */}
                            <div className="space-y-2">
