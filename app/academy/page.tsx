...
import { Search, BookOpen, Clock, Tag, Layers, GraduationCap, Zap, PlayCircle, Mic, Newspaper, ArrowLeft } from 'lucide-react';
...
function AcademyContent() {
  ...
  const parseTags = (tags: any) => {
    if (!tags) return [];
    let parsed: any[] = [];
    if (Array.isArray(tags)) parsed = tags;
    else if (typeof tags === 'string') {
      try { const json = JSON.parse(tags); if (Array.isArray(json)) parsed = json; else parsed = tags.split(/[,，]/); } 
      catch (e) { parsed = tags.split(/[,，]/); }
    }
    return parsed.map(t => typeof t === 'string' ? t.replace(/[\[\]"'\\]/g, '').trim() : '').filter(t => t && t.length > 0);
  };
      ...
      <div className="border-b border-white/5 bg-[#0A0A0A]/90 sticky top-0 z-40 backdrop-blur-xl px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="relative w-64 hidden md:block">
                ...
                <input type="text" placeholder="搜索教程..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#151515] border border-white/10 rounded-full py-2 pl-9 pr-4 text-xs focus:outline-none focus:border-purple-500 transition-all"/>
            </div>
        </div>
      </div>

      {/* ✅ 核心修改：布局从 Flex 改为 Grid，与详情页保持完全一致 */}
      <main className="max-w-7xl mx-auto p-6 md:p-8 grid grid-cols-12 gap-8">
        
        {/* 👈 左侧导航栏 */}
        {/* 关键点：lg:col-span-2 强制桌面端占位与详情页完全一致 */}
        <aside className="col-span-12 md:col-span-3 lg:col-span-2 sticky top-24 h-fit">
                        ...
                        <Link href={`/academy/${item.id}`} key={item.id} className="group flex flex-col bg-[#151515] border border-white/5 rounded-2xl overflow-hidden hover:border-purple-500/50 transition-all hover:-translate-y-1">
                            <div className="aspect-video relative overflow-hidden bg-gray-800">
                                {item.image_url ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-800 to-black"><BookOpen size={40} className="text-gray-700"/></div>
                                )}
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-white/5 mt-auto">
                                    <div className="flex gap-1 overflow-hidden flex-wrap h-6">
                                        {parseTags(item.tags).slice(0,3).map((tag:string, i:number) => (
                                            <span key={i} className="text-[10px] bg-white/5 text-gray-400 px-1.5 py-0.5 rounded flex items-center gap-1 whitespace-nowrap"><Tag size={10}/> {tag}</span>
                                        ))}
                                    </div>
                                    <div className="flex items-center gap-1 text-[10px] text-gray-500 font-mono flex-shrink-0"><Clock size={10}/> {item.duration || '5m'}</div>
                                </div>
                            </div>
                        </Link>
                    ))}
